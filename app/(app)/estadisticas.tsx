    ...
    <View style={{ flex: 1, backgroundColor: '#F9FAFB' }}>
      ...
      >
        <View style={{ paddingHorizontal: 16, paddingVertical: 16, gap: 16 }}>
          ...
          {/* ── KPIs (botones navegables) ── */}
          {(() => {
            const cardW = Math.floor((W - 32 - 12) / 2);
            const kpis = [
              { title: 'Pedidos',   value: d.stats.pedidos,   icon: 'cart-outline'   as const, color: 'primary'  as const, route: '/(app)/pedidos'              },
              { title: 'Clientes',  value: d.stats.clientes,  icon: 'people-outline' as const, color: 'info'     as const, route: '/(app)/clientes'             },
              { title: 'Ingresos',  value: `S/ ${d.stats.ingresos.toLocaleString()}`, icon: 'cash-outline' as const, color: 'success' as const, route: '/(app)/facturacion/ventas'   },
              { title: 'Productos', value: d.stats.productos,  icon: 'cube-outline'   as const, color: 'warning'  as const, route: '/(app)/almacen/productos'    },
            ];
            return (
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
                {kpis.map(k => (
                  <TouchableOpacity key={k.title} style={{ width: cardW }} activeOpacity={0.75} onPress={() => router.push(k.route as any)}>
                    <StatCard title={k.title} value={k.value} icon={k.icon} color={k.color} />
                  </TouchableOpacity>
                ))}
              </View>
            );
