      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: Math.max(bottom, 16) + 24 }}>
          <View className="p-4 gap-4">
            ...
            <Card className="p-4 bg-blue-50 border border-blue-200">
              <View className="flex-row gap-2">
                ...
                <Text variant="bodySmall" color="primary" className="flex-1">
                  Busca la factura o boleta para crear una nota de crédito por devolución.
                </Text>
              </View>
            </Card>
            ...
            {comprobanteReferencia && (
              <Card className="p-4 gap-3">
                <Text variant="label" className="mb-2">Tipo de Nota de Crédito *</Text>
            ...
            {comprobanteReferencia && (
              <Card className="p-4 gap-3">
                <Text variant="label" className="mb-2">Motivo de la Devolución *</Text>
                <TextInput
            ...
            {comprobanteReferencia && items.length > 0 && (
              <Card className="p-4 gap-3">
                <View className="flex-row items-center justify-between mb-3">
                  <Text variant="label">Productos a Devolver</Text>
            ...
            {comprobanteReferencia && items.filter(i => i.seleccionado).length > 0 && (
              <Card className="p-4 bg-gray-50">
                <View className="gap-2">
                  <View className="flex-row justify-between">
                    <Text variant="body" color="muted">Total a Devolver</Text>
                    <Text variant="h4" className="font-bold text-green-600">S/ {calcularTotal().toFixed(2)}</Text>
                  </View>
                </View>
              </Card>
            )}
            ...
            {comprobanteReferencia && (
              <Button
                onPress={async () => {
                  const ok = await save();
                  if (ok) {
                    setStatus({ visible: true, type: 'success', title: 'Nota Creada', message: 'La nota de crédito se ha registrado correctamente.' });
                  }
                }}
                disabled={saving}
                loading={saving}
