...
export function useVentas() {
  ...
  const load = useCallback(async (q: string, est: string, p: number, append = false) => {
    ...
    setError(null);
    try {
      const raw: any = await facturacionService.getVentas({
        search: q || undefined,
        estado: est || undefined,
        page: p,
      });

      if (!raw) { setError('Error al cargar ventas'); return; }

      const list: Venta[] = Array.isArray(raw?.data) ? raw.data : [];
      setVentas(prev => append ? [...prev, ...list] : list);
  ...
  return { ventas, search, setSearch, estado, applyEstado, total, loading, loadingMore, error, loadMore, refresh };
}
